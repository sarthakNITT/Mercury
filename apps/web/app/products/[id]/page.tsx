...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            <button onClick={() => router.back()} className="btn btn-outline" style={{ marginBottom: '1rem' }}>&larr; Back</button>

            <div className="card" style={{ padding: '2rem' }}>
                <div style={{ display: 'flex', gap: '2rem', flexDirection: 'column' }}>
                    <div className="card-img" style={{ height: '300px' }}>
                        {product.imageUrl ? <img src={product.imageUrl} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : 'No Image'}
                    </div>

                    <div>
                        <h1 className="title" style={{ fontSize: '2.5rem' }}>{product.name}</h1>
                        <p style={{ color: '#8b949e', fontSize: '1.2rem' }}>{product.category}</p>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                            <button onClick={() => handleAction('CLICK')} className="btn" style={{ background: '#e3b341', color: 'black' }}>
                                Simulate Click
                            </button>
                            <button onClick={() => handleAction('CART')} className="btn" style={{ background: '#d29922' }}>
                                Add to Cart
                            </button>
                            <button onClick={() => handleAction('PURCHASE')} className="btn btn-success">
                                Buy Now (Demo)
                            </button>
                        </div>
                    </div>
